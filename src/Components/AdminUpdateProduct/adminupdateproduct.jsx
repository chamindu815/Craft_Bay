...
const AdminUpdateProduct = ({
  ...
  useEffect(() => {
    if (productItem) {
      const selectedCategory = options.find(
        (o) => o.value == productItem.category
      );
  ...
  const handleDeleteRow = (id, type) => {
    if (type == "BuyingPrice") {
      setFormValues({
        ...formValues,
        adminProductBuyingPriceDetailsDtos:
          formValues.adminProductBuyingPriceDetailsDtos.filter(
            (b, idx) => idx != id
          ),
      });
    } else {
      setFormValues({
        ...formValues,
        adminProductSellingPriceDetailsDtos:
          formValues.adminProductSellingPriceDetailsDtos.filter(
            (b, idx) => idx != id
          ),
      });
    }
  ...
  const handleSubmit = (newRow) => {
    if (modalOpen == "BuyingPrice") {
      setFormValues({
        ...
        adminProductBuyingPriceDetailsDtos:
          rowToEdit === null
            ? [...formValues.adminProductBuyingPriceDetailsDtos, newRow]
            : formValues.adminProductBuyingPriceDetailsDtos.map(
                (currRow, idx) => {
      ...
      setFormValues({
        ...
        adminProductSellingPriceDetailsDtos:
          rowToEdit === null
            ? [...formValues.adminProductSellingPriceDetailsDtos, newRow]
            : formValues.adminProductSellingPriceDetailsDtos.map(
                (currRow, idx) => {
                  if (idx !== rowToEdit) return currRow;
                  return newRow;
                }
          ...
          <div className="prod-update-txtbox">
            ...
            <div>
              <Select
                ...
                className="update_category_select"
              />
              <button
                type="submit"
            ...
            <div className="prod-update-tbl">
              <table className="table">
                ...
                <tbody>
                  {formValues &&
                    formValues.adminProductBuyingPriceDetailsDtos &&
                    formValues?.adminProductBuyingPriceDetailsDtos.map(
                      (row, idx) => (
                        <tr key={idx}>
                          <td>{row.date}</td>
                          ...
                          <td>
                            <span className="actions">
                              <BsFillTrashFill
                                ...
                                onClick={() =>
                                  handleDeleteRow(idx, "BuyingPrice")
                                }
                              />
                              <BsFillPencilFill
                                onClick={() =>
                                  handleEditRow(idx, "BuyingPrice")
                                }
                              />
                            </span>
                          </td>
              ...
              <button
                className="btn"
                onClick={() => setModalOpen("BuyingPrice")}
              >
                Add
              </button>
            </div>
            ...
            <div className="prod-update-tbl">
              <table className="table">
                ...
                <tbody>
                  {formValues &&
                    formValues.adminProductSellingPriceDetailsDtos &&
                    formValues.adminProductSellingPriceDetailsDtos.map(
                      (row, idx) => (
                        <tr key={idx}>
                          <td>{row.date}</td>
                          ...
                          <td>
                            <span className="actions">
                              <BsFillTrashFill
                                className="delete-btn"
                                onClick={() =>
                                  handleDeleteRow(idx, "SellingPrice")
                                }
                              />
                              <BsFillPencilFill
                                onClick={() =>
                                  handleEditRow(idx, "SellingPrice")
                                }
                              />
                            </span>
                          </td>
              ...
              <button
                className="btn"
                onClick={() => setModalOpen("SellingPrice")}
              >
                Add
              </button>
            </div>
